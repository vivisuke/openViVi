...
- 現在（6月）は、作者がopenViViのソースコード・ドキュメント編集を行うために必要な機能実装、重要問題対処中
...
- ViVi 1.x 相当をまず実装予定（期間：1～2年？）→ ViVi 7.0.xxx としてリリース？
  ...
  - （暫定）実装優先順位：
    ...
    1. 作者がソースコード・ドキュメント編集を行うための必須機能（~~インデント・逆インデント、オートインデント、単語補完、禅コーディング、~~ 罫線モード、罫線保護編集、テキストアライン、MDIタブ切り替え、日付等入力、リナンバ、アウトライン）
