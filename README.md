...
<img src = "https://github.com/vivisuke/openViVi/blob/master/screen/MainWindow-001.png" width = 600 />
...
- ViVi 1.x 相当をまず実装予定（期間：1～2年？）→ ViVi 7.0.xxx としてリリース？
  ...
  - （暫定）実装優先順位：
    ...
    1. 基本 ex コマンド（ ~~行番号pd~~ ewsgv）
    1. 作者がドキュメント編集を行うための必須機能（~~インデント・逆インデント、オートインデント、~~ 罫線モード、罫線保護編集、テキストアライン、MDIタブ切り替え、日付等入力、リナンバ、アウトライン）
